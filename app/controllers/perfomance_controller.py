...
def fetch_web_vitals(url):
    try:
        response = requests.get(
            f"https://chromeuxreport.googleapis.com/v1/records:query?url={url}&key=YOUR_API_KEY"
        )
        data = response.json()
        return data
...
def extract_lighthouse_metrics(report):
    try:
        ...
        return {
            'Error': f'Missing expected key in Lighthouse report: {str(e)}'
        }
...
def extract_web_vitals_metrics(data):
    try:
        metrics = {
            ...
            ['percentiles']['p75'],
            'FID_real':
            data['record']['metrics']['first_input_delay']['percentiles']
            ['p75'],
            'CLS_real':
            data['record']['metrics']['cumulative_layout_shift']['percentiles']
            ['p75'],
            'FCP_real':
