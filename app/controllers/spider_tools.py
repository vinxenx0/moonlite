...
def audit_image_details(url, soup):
    ...
    for img in images:
        ...
        image_details.append({
            ...
            missing_alt_attribute,
            'Alt_Text_Over_100_Characters':
            alt_text_over_100_characters
        })

    print(url)
    print(image_details)
    return {'URL': url, 'Images': image_details}
...
def get_canonical_info(soup, url, response):
    ...
    canonical_absolute = bool(
        urlparse(canonical_url).netloc) if canonical_url else False
    ...
    canonical_http_absolute = bool(urlparse(
        canonical_http_header).netloc) if canonical_http_header else False
    canonical_http_relative = not canonical_http_absolute if canonical_http_header else False
    canonical_multiple = bool(soup.find_all('link', attrs={
        'rel': 'canonical'
    })) or (canonical_http_header and canonical_http_header != canonical_url)
    canonical_multiple_conflicting = canonical_multiple and len(
        set(canonical_url for canonical_url in soup.find_all(
            'link', attrs={'rel': 'canonical'}))) > 1
...
def get_security_info(url, response, soup):
    ...
    mixed_content = bool(
        soup.find_all(lambda tag: tag.name in ['img', 'script', 'link'] and tag
                      .get('src') and tag['src'].startswith('http://'))
    ) if soup else False
    form_url_insecure = bool(
        soup.find('form',
                  attrs={'action': lambda x: x and x.startswith('http://')
                         })) if soup else False
    form_on_http_url = bool(
        soup.find('form',
                  attrs={'action': lambda x: x and x.startswith('http://')
                         })) if soup else False
    unsafe_cross_origin_links = bool(
        soup.find_all(lambda tag: tag.name == 'a' and tag.get('target') ==
                      '_blank' and ('rel' not in tag.attrs or 'noopener' not in
                                    tag.get('rel', [])))) if soup else False
    protocol_relative_resource_links = bool(
        soup.find_all(
            lambda tag: tag.name in ['img', 'script', 'link'] and tag.get(
                'src') and tag['src'].startswith('//'))) if soup else False
    ...
    missing_x_content_type_options_header = 'x-content-type-options' not in response.headers or response.headers[
        'x-content-type-options'] != 'nosniff' if response else False
    missing_x_frame_options_header = 'x-frame-options' not in response.headers if response else False
    missing_secure_referrer_policy_header = 'referrer-policy' not in response.headers or response.headers[
        'referrer-policy'] not in [
            'no-referrer-when-downgrade', 'strict-origin-when-cross-origin',
            'no-referrer', 'strict-origin'
        ] if response else False
    bad_content_type = response.headers.get(
        'content-type', '').split(';')[0] != 'text/html' if response else False
    ...
    return {
        ...
        'Missing_Content-Security-Policy_Header':
        missing_content_security_policy_header,
        'Missing_X-Content-Type-Options_Header':
        missing_x_content_type_options_header,
        'Missing_X-Frame-Options_Header': missing_x_frame_options_header,
        'Missing_Secure_Referrer-Policy_Header':
        missing_secure_referrer_policy_header,
        'Bad_Content_Type': bad_content_type
    }


def get_common_url_issues(url):
    non_ascii_characters = any(ord(char) > 127
                               for char in url) if url else False
    ...
    repetitive_path = any(url.count(part) > 1
                          for part in url.split('/')) if url else False
    ...
    ga_tracking_parameters = any(
        param in url for param in ['utm=', '_ga=', '_gl=']) if url else False
...
def get_page_title_issues(soup):
    ...
    title_missing = not soup.find('title') or not soup.find(
        'title').text.strip() if soup else False
    all_titles = [title.text.strip()
                  for title in soup.find_all('title')] if soup else []
    title_duplicate = len(all_titles) != len(set(all_titles))
    title_over_60_characters = any(
        len(title.text.strip()) > 60
        for title in soup.find_all('title')) if soup else False
    title_below_30_characters = any(
        len(title.text.strip()) < 30
        for title in soup.find_all('title')) if soup else False
    # Over X Pixels
...
def get_meta_description_issues(soup):
    ########### META DESCRIPTION #########################
    meta_description_missing = not soup.find(
        'meta', attrs={'name': 'description'}) or not soup.find(
            'meta', attrs={
                'name': 'description'
            }).get('content').strip() if soup else False
    all_meta_descriptions = [
        meta.get('content').strip()
        for meta in soup.find_all('meta', attrs={'name': 'description'})
    ] if soup else []
    meta_description_duplicate = len(all_meta_descriptions) != len(
        set(all_meta_descriptions))
    meta_description_over_155_characters = any(
        len(meta.get('content').strip()) > 155 for meta in soup.find_all(
            'meta', attrs={'name': 'description'})) if soup else False
    meta_description_below_70_characters = any(
        len(meta.get('content').strip()) < 70 for meta in soup.find_all(
            'meta', attrs={'name': 'description'})) if soup else False
    # Over X Pixels
    ...
    meta_description_multiple = len(
        soup.find_all('meta', attrs={'name': 'description'
                                     })) > 1 if soup else False
    meta_description_outside_head = any(
        meta.find_parent('head') is None for meta in soup.find_all(
            'meta', attrs={'name': 'description'})) if soup else False
...
def get_meta_keywords_issues(soup):
    ############# KEYWORDS #######################
    keywords_missing = not soup.find('meta', attrs={'name': 'keywords'
                                                    }) if soup else True
    all_keywords = [
        keywords.get('content', '').strip()
        for keywords in soup.find_all('meta', attrs={'name': 'keywords'})
    ] if soup else []
    keywords_duplicate = len(all_keywords) != len(set(all_keywords))
    keywords_multiple = len(soup.find_all(
        'meta', attrs={'name': 'keywords'})) > 1 if soup else False
...
def get_h1_issues(soup):
    h1_missing = not soup.find('h1') or not soup.find(
        'h1').text.strip() if soup else False
    all_h1s = [h1.text.strip() for h1 in soup.find_all('h1')] if soup else []
    h1_duplicate = len(all_h1s) != len(set(all_h1s))
    h1_over_70_characters = any(
        len(h1.text.strip()) > 70
        for h1 in soup.find_all('h1')) if soup else False
    h1_multiple = len(soup.find_all('h1')) > 1 if soup else False
    alt_text_in_h1 = bool(soup.find('h1 img[alt]')) if soup else False
    non_sequential_h1 = not soup.find_all(
        ['h2', 'h3', 'h4', 'h5', 'h6']) or soup.find_all(
            ['h2', 'h3', 'h4', 'h5', 'h6'])[0].name != 'h1' if soup else False
...
def get_h2_issues(soup):
    h2_missing = not soup.find('h2') or not soup.find(
        'h2').text.strip() if soup else False
    all_h2s = [h2.text.strip() for h2 in soup.find_all('h2')] if soup else []
    h2_duplicate = len(all_h2s) != len(set(all_h2s))
    h2_over_70_characters = any(
        len(h2.text.strip()) > 70
        for h2 in soup.find_all('h2')) if soup else False
...
def get_directive_issues(soup, response):
    ...
    meta_robots_content = meta_robots_tag.get(
        'content') if meta_robots_tag else ''
    ...
    directives = {
        'Index':
        'index' in meta_robots_content.lower()
        or 'index' in x_robots_tag.lower(),
        'Noindex':
        'noindex' in meta_robots_content.lower()
        or 'noindex' in x_robots_tag.lower(),
        'Follow':
        'follow' in meta_robots_content.lower()
        or 'follow' in x_robots_tag.lower(),
        'Nofollow':
        'nofollow' in meta_robots_content.lower()
        or 'nofollow' in x_robots_tag.lower(),
        'None':
        'none' in meta_robots_content.lower()
        or 'none' in x_robots_tag.lower(),
        'NoArchive':
        'noarchive' in meta_robots_content.lower()
        or 'noarchive' in x_robots_tag.lower(),
        'NoSnippet':
        'nosnippet' in meta_robots_content.lower()
        or 'nosnippet' in x_robots_tag.lower(),
        'Max-Snippet':
        'max-snippet' in meta_robots_content.lower()
        or 'max-snippet' in x_robots_tag.lower(),
        'Max-Image-Preview':
        'max-image-preview' in meta_robots_content.lower()
        or 'max-image-preview' in x_robots_tag.lower(),
        'Max-Video-Preview':
        'max-video-preview' in meta_robots_content.lower()
        or 'max-video-preview' in x_robots_tag.lower(),
        'NoODP':
        'noodp' in meta_robots_content.lower()
        or 'noodp' in x_robots_tag.lower(),
        'NoYDIR':
        'noydir' in meta_robots_content.lower()
        or 'noydir' in x_robots_tag.lower(),
        'NoImageIndex':
        'noimageindex' in meta_robots_content.lower()
        or 'noimageindex' in x_robots_tag.lower(),
        'NoTranslate':
        'notranslate' in meta_robots_content.lower()
        or 'notranslate' in x_robots_tag.lower(),
        'Unavailable_After':
        'unavailable_after' in meta_robots_content.lower()
        or 'unavailable_after' in x_robots_tag.lower(),
        'Refresh':
        soup.find('meta', attrs={'http-equiv': 'refresh'}) is not None,
        'Outside_<head>':
        meta_robots_tag and not meta_robots_tag.find_parent('head')
    }
...
def get_hreflang_issues(soup):
    hreflang_elements = soup.find_all('link',
                                      attrs={
                                          'rel': 'alternate',
                                          'hreflang': True
                                      })
    ...
    for hreflang_element in hreflang_elements:
        ...
        hreflang_status_code = requests.head(
            hreflang_url).status_code if hreflang_url else None
        ...
        return_hreflang_url = soup.find('link',
                                        attrs={
                                            'rel': 'alternate',
                                            'href': hreflang_url
                                        })
        ...
        elif return_hreflang_url and return_hreflang_url.get(
                'hreflang') != hreflang_element.get('hreflang'):
            inconsistent_language_region_return_links.append(hreflang_url)

        canonical_href = soup.find('link',
                                   rel='canonical').get('href') if soup.find(
                                       'link', rel='canonical') else None
        ...
        if not hreflang_value or not re.match(r'^[a-z]{2}(-[A-Z]{2})?$',
                                              hreflang_value):
            incorrect_language_region_codes.append(hreflang_url)

        hreflang_entries = soup.find_all('link',
                                         attrs={
                                             'rel': 'alternate',
    ...
    return {
        ...
        'Inconsistent_Language_&_Region_Return_Links':
        inconsistent_language_region_return_links,
...
def get_structured_data_issues(soup):
    ...
    for script in structured_data:
        ...
        try:
            ...
            parse_errors.append(
                data
            )  # Almacenamos el contenido del script en lugar del objeto Tag
            continue
            ...
            if type_value:
                if 'error' in type_value:
                    validation_errors.append(
                        data)  # Almacenamos el contenido del script
                else:
...
def validate_html(html_content):
    ...
    invalid_head_elements = len(
        soup.find_all(
            lambda tag: tag.name == 'head' and tag.parent.name != 'html'))
    body_preceding_html = len(
        soup.find_all(
            lambda tag: tag.name == 'body' and tag.parent.name != 'html'))
    head_not_first = len(
        soup.find_all(lambda tag: tag.name == 'head' and tag.parent.contents[0]
                      .name != 'html'))
    missing_head_tag = 1 if not soup.find('head') else 0
    multiple_head_tags = len(soup.find_all('head')) - 1 if len(
        soup.find_all('head')) > 1 else 0
    missing_body_tag = 1 if not soup.find('body') else 0
    multiple_body_tags = len(soup.find_all('body')) - 1 if len(
        soup.find_all('body')) > 1 else 0
...
def check_gzip(url):
    try:
        response = requests.get(url)
        return 'Content-Encoding' in response.headers and (
            'gzip' in response.headers['Content-Encoding']
            or 'br' in response.headers['Content-Encoding'])
    except requests.RequestException as e:
        return f"Error: {e}"
...
def check_responsive_images(soup):
    ...
    large_images = [
        ...
        if 'width' in img.attrs and int(img['width']) > 1000
    ]
    return large_images
...
def check_ssl_expiry(url):
    try:
        ...
        expiry_date = datetime.datetime.strptime(ssl_info['notAfter'],
                                                 '%b %d %H:%M:%S %Y %Z')
...
def check_http_in_sitemap(sitemap_url, soup):
    try:
        ...
        http_urls = [
            url.text for url in urls if url.text.startswith('http://')
        ]
        return http_urls
    except requests.RequestException as e:
        return f"Error: {e}"
...
def check_mixed_content(response):
    try:
        ...
        mixed_content_elements = [
            tag['src'] for tag in soup.find_all(['img', 'script', 'link'])
...
def check_large_xml_sitemap(sitemap_url_response):
    try:
        ...
        if response.headers.get('Content-Length') and int(
                response.headers['Content-Length']) > 52428800:
            return True
...
def check_noindex_in_html_and_http(response):
    try:
        ...
        if meta_robots and 'noindex' in meta_robots[
                'content'] and header_robots and 'noindex' in header_robots:
            return True
        return False
    except requests.RequestException as e:
        return f"Error: {e}"
...
def check_nofollow_in_html_and_http(response):
    try:
        ...
        meta_robots = soup.find('meta', attrs={'name': 'robots'})
        header_robots = response.headers.get('X-Robots-Tag')
        if meta_robots and 'nofollow' in meta_robots[
                'content'] and header_robots and 'nofollow' in header_robots:
            return True
        return False
    except requests.RequestException as e:
        return f"Error: {e}"
...
def check_blocked_by_robots_txt(url):
    try:
        ...
        if 'Disallow' in robots_response.text:
            disallow_paths = [
                line.split(' ')[1] for line in robots_response.text.split('\n')
...
def check_blocked_by_x_robots_tag(response):
    try:
        ...
        header_robots = response.headers.get('X-Robots-Tag')
        return header_robots and ('noindex' in header_robots
                                  or 'nofollow' in header_robots)
    except requests.RequestException as e:
        return f"Error: {e}"
...
def check_canonical_http_to_https(url, response):
    try:
        ...
        soup = BeautifulSoup(response.content, 'html.parser')
        canonical = soup.find('link', rel='canonical')
        return canonical and canonical['href'].startswith(
            'https://') and url.startswith('http://')
    except requests.RequestException as e:
        return f"Error: {e}"
...
def check_multiple_canonical(response):
    try:
        ...
        return len(canonicals) > 1 and len(
            set([canonical['href'] for canonical in canonicals])) > 1
    except requests.RequestException as e:
        return f"Error: {e}"
...
def check_xml_sitemap_status(sitemap_url_response):
    try:
        ...
        issues = {
            '4XX': [
                url for url, code in status_codes.items()
...
def check_internal_links_status(response):
    try:
        ...
        issues = {
            '3XX': [
                link for link, code in status_codes.items()
                if str(code).startswith('3')
            ],
            '4XX': [
                link for link, code in status_codes.items()
...
def check_hreflang_status(response):
    try:
        ...
        soup = BeautifulSoup(response.content, 'html.parser')
        hreflangs = [
            link['href'] for link in soup.find_all('link', rel='alternate')
        ]
        status_codes = {
            hreflang: check_status_code(hreflang)
            for hreflang in hreflangs
        }
        issues = {
            '3XX': [
                hreflang for hreflang, code in status_codes.items()
                if str(code).startswith('3')
            ],
            '4XX': [
                hreflang for hreflang, code in status_codes.items()
...
def check_images_status(response):
    try:
        ...
        issues = {
            '3XX': [
                img for img, code in status_codes.items()
                if str(code).startswith('3')
            ],
            '4XX': [
                img for img, code in status_codes.items()
...
def check_javascript_status(response):
    try:
        ...
        status_codes = {
            script: check_status_code(script)
            for script in scripts
        }
        issues = {
            '3XX': [
                script for script, code in status_codes.items()
                if str(code).startswith('3')
            ],
            '4XX': [
                script for script, code in status_codes.items()
...
def check_css_status(response):
    try:
        ...
        status_codes = {
            stylesheet: check_status_code(stylesheet)
            for stylesheet in stylesheets
        }
        issues = {
            '3XX': [
                stylesheet for stylesheet, code in status_codes.items()
                if str(code).startswith('3')
            ],
            '4XX': [
                stylesheet for stylesheet, code in status_codes.items()
...
def __get_url_details(url_info):
    ...
    url_details = {
        ...
        url_info.get('url'),
        'Content':
        url_info.get('Content', ''),
        'Status_Code':
        url_info.get('Status Code', ''),
        'Status':
        url_info.get('Status', ''),
        'Indexability':
        url_info.get('Indexability', ''),
        'Indexability_Status':
        url_info.get('Indexability Status', ''),
        'Title_1':
        url_info.get('Title 1', ''),
        'Title_1_Length':
        url_info.get('Title 1 Length', 0),
        'Title_1_Pixel_Width':
        url_info.get('Title 1 Pixel Width', 0),
        'Meta Description_1':
        url_info.get('Meta Description 1', ''),
        'Meta Description_Length_1':
        url_info.get('Meta Description Length 1', 0),
        'Meta Description_Pixel_Width':
        url_info.get('Meta Description Pixel Width', 0),
        'Meta Keyword_1':
        url_info.get('Meta Keyword 1', ''),
        'Meta Keywords_Length':
        url_info.get('Meta Keywords Length', 0),
        'h1_-_1':
        url_info.get('h1 - 1', ''),
        'h1_-_Len-1':
        url_info.get('h1 - Len-1', 0),
        'h2_-_1':
        url_info.get('h2 - 1', ''),
        'h2_-_Len-1':
        url_info.get('h2 - Len-1', 0),
        'Meta_Robots_1':
        url_info.get('Meta Robots 1', ''),
        'X-Robots-Tag_1':
        url_info.get('X-Robots-Tag 1', ''),
        'Meta_Refresh_1':
        url_info.get('Meta Refresh 1', ''),
        'Canonical_Link_Element':
        url_info.get('Canonical Link Element', ''),
        'rel="next"_1':
        url_info.get('rel="next" 1', ''),
        'rel="prev"_1':
        url_info.get('rel="prev" 1', ''),
        'HTTP_rel="next"_1':
        url_info.get('HTTP rel="next" 1', ''),
        'HTTP_rel="prev"_1':
        url_info.get('HTTP rel="prev" 1', ''),
        'Size':
        url_info.get('Size', 0),
        'Transferred':
        url_info.get('Transferred', 0),
        'Word_Count':
        url_info.get('Word Count', 0),
        'Text_Ratio':
        url_info.get('Text Ratio', 0),
        'Crawl_Depth':
        url_info.get('Crawl Depth', 0),
        'Folder_Depth':
        url_info.get('Folder Depth', 0),
        'Link_Score':
        url_info.get('Link Score', 0),
        'Inlinks':
        url_info.get('Inlinks', 0),
        'Unique_Inlinks':
        url_info.get('Unique Inlinks', 0),
        'Unique_JS_Inlinks':
        url_info.get('Unique JS Inlinks', 0),
        '%_of_Total':
        url_info.get('% of Total', 0),
        'Outlinks':
        url_info.get('Outlinks', 0),
        'Unique_Outlinks':
        url_info.get('Unique Outlinks', 0),
        'Unique_JS_Outlinks':
        url_info.get('Unique JS Outlinks', 0),
        'External_Outlinks':
        url_info.get('External Outlinks', 0),
        'Unique_External_Outlinks':
        url_info.get('Unique External Outlinks', 0),
        'Unique_External_JS_Outlinks':
        url_info.get('Unique External JS Outlinks', 0),
        'Closest_Similarity_Match':
        url_info.get('Closest Similarity Match', ''),
        'No._Near_Duplicates':
        url_info.get('No. Near Duplicates', 0),
        'Spelling_Errors':
        url_info.get('Spelling Errors', 0),
        'Grammar_Errors':
        url_info.get('Grammar Errors', 0),
        'Language':
        url_info.get('Language', ''),
        'Hash':
        url_info.get('Hash', ''),
        'Response Time':
        url_info.get('Response Time', ''),
        'Last-Modified':
        url_info.get('Last-Modified', ''),
        'Redirect_URI':
        url_info.get('Redirect URI', ''),
        'Redirect_Type':
        url_info.get('Redirect Type', ''),
        'HTTP_Version':
        url_info.get('HTTP Version', ''),
        'URL_Encoded Address':
        url_info.get('URL Encoded Address', ''),
        'Title_2':
        url_info.get('Title 2', ''),
        'Meta_Description 2':
        url_info.get('Meta Description 2', ''),
        'h1-2':
        url_info.get('h1-2', ''),
        'h2-2':
        url_info.get('h2-2', '')
...
def get_page_info(url):
    ...
    if soup:
        try:
            ...
            spelling_errors, grammar_errors = analizar_ortografia(
                response.text)
