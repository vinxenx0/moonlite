...
def analyze_text_content(html_content):
    ...
    h1_issues = {
        ...
        len(h1_tags) == 0,
        'empty':
        any(not tag.get_text().strip() for tag in h1_tags),
        'too_long':
        any(len(tag.get_text()) > 60 for tag in h1_tags),
        'multiple':
        len(h1_tags) > 1,
        'duplicate':
        any(tag.get_text().strip() == h1_tags[0].get_text().strip()
            for tag in h1_tags[1:]),
        'identical_title':
        soup.title
        and any(tag.get_text().strip() == soup.title.get_text().strip()
                for tag in h1_tags)
    }
...
def text_content_audit(url):
    try:
        ...
        return {
            'Text Content Issues': issues
        } if issues else {
            'Text Content Issues': 'No issues found'
        }
    except Exception as e:
        return {'Error': str(e)}
