...
class SubscriptionForm(FlaskForm):
    subscription_plan = SelectField('Plan de Suscripción',
                                    choices=[('Free', 'Free - 0$'),
                                             ('Pro', 'Pro - 9$'),
                                             ('Corporate', 'Corporate - 129$')
                                             ],
                                    validators=[DataRequired()])
    subscription_currency = StringField('Moneda (ISO, e.g., USD, EUR)',
                                        validators=[DataRequired()])
    subscription_frequency = SelectField('Frecuencia de Pago',
                                         choices=[('Monthly', 'Mensual'),
                                                  ('Annually', 'Anual')],
                                         validators=[DataRequired()])
...
class NewUserRegistrationForm(FlaskForm):
    ...
    subscription_plan = SelectField('Plan de suscripción',
                                    ...
                                    default='Free',
                                    validators=[DataRequired()])
    payment_frequency = SelectField('Frecuencia de pago',
                                    choices=[('Monthly', 'Mensual'),
                                             ('Annually', 'Anual')],
                                    validators=[DataRequired()],
                                    default='Monthly')
    ...
    def validate_email(self, email):
        ...
        if user:
            raise ValidationError(
                'Este correo electrónico ya está registrado. Por favor, use otro.'
            )
    ...
    def validate_payment_frequency(self, payment_frequency):
        ...
        if self.subscription_plan.data == 'Free' and payment_frequency.data:
            raise ValidationError(
                'La frecuencia de pago no es necesaria para el plan gratuito.')
...
class ConfigForm(FlaskForm):
    ...
    logo_url = StringField('URL del logotipo',
...
class SeoToolsForm(FlaskForm):
    domain = StringField('Página analizar incluyendo http o https',
                         validators=[DataRequired()])
    submit = SubmitField('Enviar')
