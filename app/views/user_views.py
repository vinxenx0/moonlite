...
def calculate_log_statistics(logs):
    ...
    monthly_logs = [
        log for log in logs if log.timestamp.year == today.year
        and log.timestamp.month == today.month
    ]
...
def usuarios():
    ...
    return render_template('user/usuarios.html',
                           users=users,
                           breadcrumbs=breadcrumbs)
